...
- `pnpm data:reset:all -- --confirm`：清除本地工作区的任务、任务分组、目标、依赖、重复模板、复盘卡片、事件、AI 运行、卡片、对话、消息及规则配置；保留数据库结构、工作区和用户，然后恢复三条默认规则。
...
- `pnpm data:inspect -- --limit 50`：打印任务、任务分组、附件数量与总字节、目标、依赖、重复模板、复盘卡片总量，状态/温度分布及最近事件。
